    ...
    <>
      <div className="mt-6 bg-white p-6 rounded-lg shadow-sm border">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h2 className="text-xl font-bold text-gray-800">Daftar Toko Besar</h2>
        <p className="text-sm text-gray-500">Kelola data toko besar stock pangan</p>
          </div>
          <button onClick={handleOpenCreateModal} className="btn-primary">
            <Plus size={16} className="mr-2" />Tambah Toko Besar
          </button>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Toko Besar</th>
        ...
        {filteredDistributors.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            {searchTerm ? 'Tidak ada toko besar yang sesuai dengan pencarian.' : 'Belum ada data toko besar.'}
          </div>
        )}
      </div>

      {/* Modal Form */}
      <Modal isOpen={isFormModalOpen} onClose={() => setIsFormModalOpen(false)} title={editingDistributor ? 'Edit Toko Besar' : 'Tambah Toko Besar'}>
        <form onSubmit={handleFormSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Nama Toko Besar *</label>
            <input
              type="text"
              required
      ...
      <ConfirmationModal
        ...
        title="Hapus Toko Besar"
        message={`Apakah Anda yakin ingin menghapus toko besar "${distributorToDelete?.nama_distributor}"? Tindakan ini tidak dapat dibatalkan.`}
      />
    </>
  );
}